...
const CIList = () => {
  ...
  const fetchData = () => {
    const url = "administration/cognitive_solutions/programs/cognitive_internships/";
  ...
  const searchSubmitHandler = (event) => {
    ...
    const url = `administration/cognitive_solutions/programs/cognitive_internships/?search=${searchItem}`;
    const token = localStorage.getItem('token');
    const headers = {
      headers: {
        Authorization: ` Token ${token}`,
                    ...
                    {list.map((listItem) => {
                      return (
                        <tr
                          className="tableRow"
                          onClick={() => {
                            history.push(`cognitive_internship/${listItem.id}`);
                          }}
                        >
                          ...
                          {/* <td>{internships.map(internship => internship.id === listItem.internship && internship.name)}</td> */}
