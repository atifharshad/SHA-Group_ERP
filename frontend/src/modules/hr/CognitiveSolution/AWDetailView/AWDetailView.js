...
const AWDetailView = (props) => {
  ...
  const fetchData = () => {
    ...
    axios
      .get(
        `administration/cognitive_solutions/programs/academic_workshops/${id}/`,
        headers
      )
      .then((res) => {
        console.log(res.data);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      const id = props.match.params.id;
      const url = `administration/cognitive_solutions/programs/academic_workshops/${id}/`;
      const token = localStorage.getItem("token");
      const headers = {
        headers: {
          Authorization: ` Token ${token}`,
                ...
                <Row>
                  <Col className="pr-1" md="6">
                    <FormGroup>
                      ...
                      <Input type="select" disabled {...formik.getFieldProps("college")}>
                        {colleges.map((college) =>
                          detail.college === college.id ? (
