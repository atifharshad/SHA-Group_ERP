...
const EIList = () => {
  ...
  const fetchData = () => {
    const url = "administration/cognitive_solutions/programs/employee_internships/";
  ...
  const searchSubmitHandler = (event) => {
    ...
    const url = `administration/cognitive_solutions/programs/employee_internships/?search=${searchItem}`;
    const token = localStorage.getItem('token');
    const headers = {
      headers: {
        Authorization: ` Token ${token}`,
                    ...
                    {list.map((listItem) => {
                      return (
                        <tr
                          className="tableRow"
                          onClick={() => {
                            history.push(`employee_internship/${listItem.id}`);
                          }}
                        >
