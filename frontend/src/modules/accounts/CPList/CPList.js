...
const SPList = ({url}) => {
  // console.log("url from AIList", url)
  ...
  const fetchData = () => {
    ...
    // }
    axios
      .get('projects/dataqueue_systems/client_projects/projects/', headers)
      .then((res) => {
        console.log(res.data);
        setList(res.data);
      })
  ...
  const paymentSubmitHandler = (event) => {
    ...
    console.log(data);
    axios
      .post("accounts/all/fees/client_project_bill/", data, headers)
      .then((res) => {
        console.log(res.data);
                    ...
                    {list.map((listItem) => {
                      return (
                        <tr className="tableRow">
                          ...
                          {company.map((item) => {
                            return item.id == listItem.company ? (
                              <td>{item.name}</td>
                            ): (null)
                            })}       
                            ...
                            {listItem.is_paid ? (
                              ...
                              <Button
                                onClick={() => {
                                  ...
                                  setStudentName(listItem.client_name);
                                  setModalOpen(true);
                                }}
                              >
                                PAY NOW
                          ...
                          <td>
                           <Button
                                onClick={() => { 
                                  ...
                                  history.push({
                                    pathname: `/accounts/reciept_list/${'CLIENT PROJECT'}/${listItem.id}`,
                                    // search: '?query=abc',
                                });
                                  // history.push(`${url.url}/receipt/${'ACADEMIC INTERNSHIP'}/${listItem.id}`)
                                }
