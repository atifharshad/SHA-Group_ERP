...
const Reciept = (props) => {
  ...
  const fetch_client_project_data = (id, headers) => {
    axios
      .get(
        `accounts/all/fees/client_project_bill/${id}`,
        headers
      )
      .then((res) => {
        setDetail(res.data);
        ...
        axios
      .get(
        `projects/dataqueue_systems/client_projects/projects/${res.data.received_from}/`,
        headers
      )
      .then((res) => {
      // alert(res.data.name);
      setReceivedFrom(res.data.client_name)
      })
      .catch((err) => console.log(err));
  ...
  const fetchData = () => {
    ...
    props.match.params.type === "ACADEMIC WORKSHOP" && 
      ...
      props.match.params.type === "CLIENT PROJECT" && 
      fetch_client_project_data(id, headers);
