...
class EmployeeInternshipFees(InternshipFeesCleanMixin, Fees):
    ...
    def __str__(self):
        return self.receipt_no + ':' + self.received_from.name if self.received_from else "unknown"
...
class StudentProjectFees(StudentProjectFeesCleanMixin, Fees):
    ...
    def balance(self):
        ...
        balance = self.received_from.actual_fees - self.received_from.concession - paid_amount
        return balance 

class ClientProjectBill(Fees):
    received_from = models.ForeignKey(ClientProject, on_delete=models.SET_NULL, null=True) 

    def __str__(self):
        return self.receipt_no  + ':' + self.received_from.client_name
    ...
    def balance(self):
        all_receipt = self._meta.model.objects.filter(received_from=self.received_from)
        paid_amount = self.paid_amount 
        balance = self.received_from.cost - self.received_from.concession - paid_amount
        return balance
    ...
    def clean(self):
        ...
        project_cost = self.received_from.cost - self.received_from.concession
        # all_receipt = CognitiveInternshipFees.objects.filter(received_from=self.received_from)
        all_receipt = self._meta.model.objects.filter(received_from=self.received_from)
        paid_amount = 0
        now_paying = self.amount
        for receipt in all_receipt:
            if receipt.id != self.id:
                paid_amount += receipt.amount
        # print('clean', paid_amount - self.amount)
        if paid_amount + now_paying > project_cost:
            raise ValidationError(
            _('Total project amount paid (%(fees_amount)s) exceed the project cost (%(project_cost)s)'),
            code='invalid_fees_amount',
            params={'fees_amount': paid_amount+now_paying, 'project_cost': project_cost}
            )   
...
def set_clien_project_is_paid(sender, instance, **kwargs):
    if not instance.received_from:
        return 

    if instance.received_from.is_paid == True:
        instance.received_from.is_paid = False 
    all_receipt = sender.objects.filter(received_from=instance.received_from)
    amount = 0
    for receipt in all_receipt:
        amount += receipt.amount
    if amount == instance.received_from.cost - instance.received_from.concession:
        instance.received_from.is_paid = True
...
pre_save.connect(set_receipt_no, sender=ClientProjectBill)
...
post_save.connect(set_clien_project_is_paid, sender=ClientProjectBill)
...
post_delete.connect(set_clien_project_is_paid, sender=ClientProjectBill)