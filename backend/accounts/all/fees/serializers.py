...
class ClientProjectBillSerializer(DynamicFieldsMixin, IdInValidationMixin, serializers.ModelSerializer):
    class Meta(IdInValidationMixin.Meta):
        model = ClientProjectBill 
        fields = '__all__'
...
class StudentProjectFeesBalanceSerializer(DynamicFieldsMixin, IdInValidationMixin, serializers.ModelSerializer):
    ...
    actual_fees = serializers.ReadOnlyField(source='received_from.actual_fees')
